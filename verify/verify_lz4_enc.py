...
def run_compress (input_file, output_file, level) :
    command_format = r'..\tinyZZZ.exe -c --lz4 %s %s'
...
def run_decompress (input_file, output_file) :
    with open(input_file, "rb") as inf:
        with open(output_file, "wb") as outf:
            outf.write(lz4.frame.decompress(inf.read()))
...
if __name__ == '__main__' :
    ...
    for in_fname in os.listdir(INPUT_DIR) :
        ...
        if os.path.isfile(in_fname_full) :
            ...
            ret_code = run_compress(in_fname_full, cmp_fname_full, level=LEVEL)
            run_decompress(cmp_fname_full, out_fname_full)
